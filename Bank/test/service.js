angular.module('ngCRUD.services', ['ngCRUD.config'])
// https://stackoverflow.com/questions/6617780/how-to-call-parent-constructor
// http://www.angelfire.com/tx4/cus/shapes/javascript.html
// https://stackoverflow.com/questions/28487909/extending-prototypes-in-javascript-good-way

.factory('DirectiveFlow', ['$timeout', 'config', 'TemplateService', function($timeout, config, TemplateService){
	
    function DirectiveFlow(scope, ctrl, attrs){
		this.FactoryName = "DirectiveFlow";
		
		this.scope = scope;
		this.ctrl = ctrl;
		this.EventListener = null;

		this.Initialize = function Initialize(scope, ctrl, attrs) {
			this.scope = scope ? scope : {};
			this.ctrl = ctrl ? ctrl : {};
			
			this.TryToCallEventListener()
			this.InitScope()
		}

		this.InitScope = function InitScope() {
		}
			
		this.TryToCallEventListener = function(){
			if(typeof this.EventListener == "function"){
				this.EventListener(this.scope, this.element, this.attrs, this.ctrl);
			}
		}
		this.Initialize();
	}
...
.factory('EntryFlow', ['DirectiveFlow', function(DirectiveFlow){	
	EntryFlow.prototype = Object.create(DirectiveFlow.prototype)
	EntryFlow.prototype.constructor = EntryFlow;
	
    function EntryFlow(scope, ctrl, attrs){
		DirectiveFlow.call(this, scope, ctrl, attrs);
	
		this.FactoryName = "EntryFlow";
		this.InitDirective = null;
		...
		this.Initialize = function Initialize(scope, ctrl, attrs) {
			this.scope = scope ? scope : {};
			this.ctrl = ctrl ? ctrl : {};
			this.element = attrs.element ? attrs.element : {};
			this.attrs = attrs.attrs ? attrs.attrs : {};
		...
		this.InitDirective = function(){
			if(this.TryToCallSetDefaultValue())
				this.SetDefaultValue(this.scope, this.element, this.attrs, this.ctrl);
		}
		this.TryToCallSetDefaultValue = function(){
		...
		this.TryToCallStatusChange = function(){
			if(typeof this.StatusChange == "function"){
				this.StatusChange($scope, $element, $attrs, $ctrl);
			}else{
				this.DefaultStatusChange($scope, $element, $attrs, $ctrl);
			}
		}
		this.DefaultStatusChange = function(){
		}
	}
...
.service('EntryBase', ['EntryFlow', function(EntryFlow){
	var EntryBase = this;
	
	EntryBase.flowObj = new EntryFlow();
	EntryBase.GetFlowObj = function(){
		return EntryBase.flowObj;
	}
	EntryBase.InitFlowObj = function(newFlowObj){
		if(typeof (newFlowObj) == "undefined" )
			EntryBase.flowObj = new EntryFlow();
		else
			EntryBase.flowObj = newFlowObj;
	}
	EntryBase.SetFlowObj = function(newFlowObj){
		EntryBase.flowObj = newFlowObj;
	}
}])

.factory('InquiryFlow', ['EntryFlow', function(EntryFlow){	
	InquiryFlow.prototype = Object.create(EntryFlow.prototype)
	InquiryFlow.prototype.constructor = InquiryFlow;
	
    function InquiryFlow(scope, ctrl, attrs){
		EntryFlow.call(this, scope, ctrl, attrs);
	
		this.FactoryName = "InquiryFlow";
		// add the variable related to flow
		// this.yourVariable = 0;
	}
	
    return InquiryFlow;
}])
.service('InquiryBase', ['InquiryFlow', function(InquiryFlow){
	var InquiryBase = this;
	
	InquiryBase.flowObj = new InquiryFlow();
	InquiryBase.GetFlowObj = function(){
		return InquiryBase.flowObj;
	}
	InquiryBase.InitFlowObj = function(newFlowObj){
		if(typeof (newFlowObj) == "undefined" )
			InquiryBase.flowObj = new InquiryFlow();
		else
			InquiryBase.flowObj = newFlowObj;
	}
	InquiryBase.SetFlowObj = function(newFlowObj){
		InquiryBase.flowObj = newFlowObj;
	}
}])

.factory('NavigationFlow', ['EntryFlow', function(EntryFlow){	
	NavigationFlow.prototype = Object.create(EntryFlow.prototype)
	NavigationFlow.prototype.constructor = NavigationFlow;
	
    function NavigationFlow(scope, ctrl, attrs){
		EntryFlow.call(this, scope, ctrl, attrs);
	
		this.FactoryName = "NavigationFlow";
	}
	
    return NavigationFlow;
}])
.service('NavigationBase', ['NavigationFlow', function(NavigationFlow){
	var NavigationBase = this;
	
	NavigationBase.flowObj = new NavigationFlow();
	NavigationBase.GetFlowObj = function(){
		return NavigationBase.flowObj;
	}
	NavigationBase.InitFlowObj = function(newFlowObj){
		if(typeof (newFlowObj) == "undefined" )
			NavigationBase.flowObj = new NavigationFlow();
		else
			NavigationBase.flowObj = newFlowObj;
	}
	NavigationBase.SetFlowObj = function(newFlowObj){
		NavigationBase.flowObj = newFlowObj;
	}
}])

.service('Core', ['$rootScope', 'config', 'SysMessageManager', function($rootScope, config, SysMessageManager){
	var core = this;
...
.service('ThemeService', ['$rootScope', 'config', 'TemplateService', function($rootScope, config, TemplateService){
	var theme = this;
...
.service('LockManager', ['$rootScope', '$timeout', 'config', function($rootScope, $cookies, config) {
	var locker = this;
	locker.lockArea = {};
	locker.tagName = "";
	locker.programId = "";
...
.service('Security', ['$rootScope', 'Core', 'CookiesManager', 'MessageService', function($rootScope, Core, $jqCookies,  MessageService) {
	var secure = this;
...
.service('CookiesManager', ['$rootScope', '$cookies', function($rootScope, $cookies) {
	var cookies = this;
	var rootScope = $rootScope;
...
.service('SysMessageManager', ["$rootScope", "$log", "config", function($rootScope, $log, config) {
	var message = this;
	var rootScope = $rootScope;
...
.service('TableManager', ["$rootScope", "$log", "config", "$q", "Security", "HttpRequeset", "DataAdapter", "Core", function($rootScope, $log, config, $q, Security, HttpRequeset, DataAdapter, Core) {
	var table = this;
	var rootScope = $rootScope;
...
.factory('LoadingModal', ['', function () {
    ...
    root.showModal = function(msg){
		...
		loadingIconContainer.endTo("body");
...
*/
.service('HttpRequeset', ['$rootScope', '$http', function($rootScope, $http){
	var self = this;
...
.service('TemplateService', ['HttpRequeset', function(HttpRequeset){
	var tpService = this;
...
.service('DataAdapter', function($rootScope, $q, HttpRequeset, DataAdapterMySQL, Security, config){
	var adapter = this;
	var dAdapter = null;
...
.service('DataAdapterMySQL', function($rootScope, Security, Core){
	var dataMySQL = this;
