...
class ExcelManager {
	...
	function Initialize($_metaTitle=""){
        /*
    ...
    function LoadTemplate(){
        ...
        $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($this->GetTemplateLocation());
    ...
    protected function CreateSpreadsheet(){
        ...
        $_objPHPExcel = new Spreadsheet();
		$_objPHPExcel->getProperties()->setCreator("PIMS")
		->setTitle($this->metaFileTitle)
		->setCategory("Utility Import/Export Template")
        ->setDescription("This document involved ".count($this->tableList)." table(s).");
        
        $this->SetSpreadsheet($_objPHPExcel);
        return $_objPHPExcel;
    }
	...
	function PrepareExportingData(&$objPHPExcel, $currentTableName){
        ...
        // Create a new worksheet named as "TableName"
        $myWorkSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($objPHPExcel, $currentTableName);
        // Attach the "myWorkSheet" worksheet as the first worksheet in the Spreadsheet object
...
//        $objPHPExcel->getDefaultStyle()->getFont()->setName('Arial Unicode MS')->setSize(12);
		...
		foreach ($toBeExportHeaderSequence as $key => $headerColumn) {
            ...
            // 20200605, keithpoon
            // fixed: stringFromColumnIndex($columnIndex) to stringFromColumnIndex(($columnIndex+1))
			...
			$excelCellCoordinate = Coordinate::stringFromColumnIndex(($columnIndex+1)).$rowCount;
			$excelColumnCoordinate = Coordinate::stringFromColumnIndex(($columnIndex+1)).":".Coordinate::stringFromColumnIndex(($columnIndex+1));
				...
				foreach($resultSet['data'] as $rowIndex => $row){
					...
					foreach($tempExportedColumnOrder as $key => $colName)
					{
						...
						if(Core::IsSystemField($colName)) // skip export value when system fields
							continue;
                        $excelCellCoordinate = Coordinate::stringFromColumnIndex(($columnIndex+1)).$rowCount;
                        // echo "columnIndex:".$columnIndex.", rowCount:".$rowCount ;
                        // echo "excelCellCoordinate:".$excelCellCoordinate;
							...
							if( $tempColValue != NULL ){
								...
								if($isDateValue){
                                    if(!$isEmptyDate){
                                        ...
                                        $objPHPExcel->getActiveSheet()->setCellValue($excelCellCoordinate, \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($tempColValue));
	...
	function Export(){
		...
		$objPHPExcel = $this->CreateSpreadsheet();
...
// 		$excelArray['Associative'] = array();
		...
		$objReader = \PhpOffice\PhpSpreadsheet\IOFactory::createReaderForFile($readExcelPath);
		...
		$_noOfUsedCols = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::columnIndexFromString($_noOfUsedColsInString);
					...
					if($columnInfo){
						switch($type){
							...
							case "timestamp":
								...
								$cellValue = date($format, \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($dateTimeValue));
